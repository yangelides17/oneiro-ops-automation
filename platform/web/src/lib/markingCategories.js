...
export function pickLayout(item) {
  ...
  if (String(item.workType || '').toLowerCase() === 'mma') return 'mma'
  return 'default'
}

export function rowRequiresColor(item) {
  if (MMA_CATEGORIES.has(item.category || '')) return true
  return String(item.workType || '').toLowerCase() === 'mma'
}

export function rowIsCompletable(item) {
  if (item.status === 'Completed' || item.status === 'completed') return true
  const qty = parseFloat(item.quantity)
  if (isNaN(qty) || qty <= 0) return false
  // Unit can come from the item itself or be auto-derived from the category
  const unit = String(item.unit || '').trim() || unitForCategory(item.category)
  if (!unit) return false
  if (rowRequiresColor(item) && !String(item.colorMaterial || '').trim()) return false
  return true
}
