...
export default function SignInHeaderCard({ fileId, filename }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/signin/header/${encodeURIComponent(fileId)}?${qs}`)
      .then(r => r.json())
      .then(json => {
        if (cancelled) return
      ...
      <div className="card p-4 space-y-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <p className="font-semibold text-navy">
              {header.contractNum || header.contractNum} · {header.regionCode || header.regionCode}
            </p>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            <p className="text-sm font-semibold text-navy">{header.contractorName || header.contractorName || '—'}</p>
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          {header.crewChief && (
            <div>
              ...
              <p className="text-sm font-semibold text-navy">{header.crewChief}</p>
            </div>
          )}
        </div>
      </div>
