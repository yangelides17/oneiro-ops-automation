...
export default function WODocsQueue({ wos, qbConnected, onDocsChange, onInvoiced }) {
  ...
  const contractors = useMemo(
    () => [...new Set(pending.map(w => w.contractorName).filter(Boolean))].sort(),
    [pending]
  )
  const rows = useMemo(
    () => pending.filter(w => contFilt.size === 0 || contFilt.has(w.contractorName)),
    [pending, contFilt]
  )
      ...
      {rows.length === 0 ? (
        ...
        <div className={expanded ? 'overflow-x-auto' : 'overflow-auto max-h-[15rem]'}>
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map(wo => (
                <tr key={wo.woNumber} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-2.5 px-3">
                    <DocStatusChips woId={wo.woNumber} docs={wo.docs} onChange={onDocsChange} />
                  </td>
                  <td className="py-2.5 px-3">
