...
export default function EditCoordinatesModal({ wo, onSaved, onClose }) {
  ...
  const save = async () => {
    ...
    setSubmitting(true)
    try {
      const res = await fetch(`/api/wos/${encodeURIComponent(wo.woId)}`, {
        method:  'PATCH',
      ...
      onSaved?.({ latitude: la, longitude: ln })
      onClose?.()
    } catch (e) {
      ...
      >
        <div className="text-center space-y-1.5">
          ...
          <p className="text-slate-500 text-sm">
            <span className="font-mono font-bold text-navy">{wo.woId}</span>
        ...
        {wo.geocodeWarning && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-800">
            <strong>Existing warning:</strong> {wo.geocodeWarning}
          </div>
        )}
