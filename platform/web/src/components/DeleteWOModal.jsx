...
export default function DeleteWOModal({ wo, onDeleted, onClose }) {
  ...
  const doDelete = async () => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/wos/${encodeURIComponent(wo.woNumber)}`, { method: 'DELETE' })
      ...
      >
        ...
        <div className="text-center space-y-1.5">
          ...
          <p className="text-slate-600 text-sm">
            <span className="font-mono font-bold text-navy">{wo.woNumber}</span>
        ...
        <div className="flex flex-col gap-2 pt-1">
          <button
            ...
            className="w-full py-3 rounded-xl font-bold text-sm bg-red-500 text-white
                       ...
                       disabled:opacity-40"
          >
            {submitting ? 'Deleting…' : `Delete WO ${wo.woNumber}`}
          </button>
          <button
            type="button"
            disabled={submitting}
