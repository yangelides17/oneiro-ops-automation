...
const rowToEdit = (r) => ({
  ...
  timeIn:         to24h(r.timeIn),
  timeOut:        to24h(r.timeOut),
...
export default function SignInHoursEditor({ fileId, filename, onDirtyChange }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/signin/rows/${encodeURIComponent(fileId)}?${qs}`)
      .then(r => r.json())
      .then(json => {
        if (cancelled) return
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/signin/rows/${encodeURIComponent(fileId)}/edit`, {
        method:  'POST',
