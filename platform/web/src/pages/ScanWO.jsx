...
export default function ScanWO() {
  ...
  const refreshCommitted = async () => {
    try {
      const res  = await fetch('/api/tools/scan-uploads-today')
  ...
  const uploadOne = async (readyItem) => {
    ...
    try {
      ...
      const res  = await fetch('/api/tools/scan-wo', { method: 'POST', body: form })
      const data = await res.json().catch(() => ({}))
      if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`)
      // If the response already contains a parsed WO (direct Vision mode),
      // skip the polling phase and go straight to 'done'.
      if (data.woNumber && !data.error) {
        setHistoryItems(prev => prev.map(p => p.id === historyId
          ? { ...p, uploadStatus: 'uploaded', parseStatus: 'done',
              fileId: data.storageKey, woIds: [data.woNumber],
              isCommitted: true }
          : p))
        return 'uploaded'
      }
      if (data.error) throw new Error(data.error)
      // Legacy queue mode: poll for status
      setHistoryItems(prev => prev.map(p => p.id === historyId
        ? { ...p, uploadStatus: 'uploaded', parseStatus: 'parsing', fileId: data.fileId }
        : p))
      return 'uploaded'
    } catch (err) {
      setHistoryItems(prev => prev.map(p => p.id === historyId
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        const res  = await fetch('/api/tools/scan-status', {
          method:  'POST',
        ...
        setHistoryItems(prev => prev.map(p => {
          ...
          const s = data.statuses.find(x => x.fileId === p.fileId)
          if (!s) return p
          if (s.status === 'done') {
            anyDone = true
