  ...
  return (
    <button
      ...
      <div className="text-sm font-semibold text-navy">
        {entry.contractNum || entry.contractNum} · {entry.regionCode || entry.regionCode}
      </div>
      {entry.contractorName && (
        <div className="text-[12px] text-slate-500 mt-0.5">{entry.contractorName}</div>
      )}
      {entry.crewChief && (
        <div className="text-[12px] text-slate-600 mt-0.5">
          <span className="font-semibold">Crew:</span> {entry.crewChief}
        </div>
      )}
...
export default function SignIn() {
    ...
    try {
      ...
      showToast(
        json.duplicate
          ...
          : `Sign-in submitted for ${selected.contractNum}`,
        'success'
      )
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    if (!draft.shiftDateEdited) {
      try {
        const checkRes = await fetch('/api/signin/check-continuation', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            timeIn:      crewClean[0].timeIn,
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="font-semibold text-navy">
                    {selected.contractNum || selected.contractNum} · {selected.regionCode || selected.regionCode}
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-sm font-semibold text-navy">{selected.contractorName || selected.contractorName || '—'}</p>
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                {selected.crewChief && (
                  <div>
                    ...
                    <p className="text-sm font-semibold text-navy">{selected.crewChief}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="card p-4 space-y-4">
              ...
              {selected.crewChief && (
                <div className="rounded-lg bg-navy/5 border border-navy/10 px-3 py-2 text-[12px]">
                  ...
                  <span className="text-slate-700">{selected.crewChief}</span>
