...
const canRegenerate = (item) => !!item && REGENERABLE_TYPES.has(item.docType)
...
export default function Approvals() {
  ...
  const handleProcessConfirm = async () => {
    const res  = await fetch('/api/tools/process-approved', { method: 'POST' })
  ...
  const filtered = useMemo(() => {
    ...
    return approvals.filter(a => a.docType === filter)
  }, [approvals, filter])
  ...
  const removeApprovedAndAdvance = (fileId) => {
    const idx = filtered.findIndex(a => a.id === fileId)
    const nextItem = filtered[idx + 1] || filtered[idx - 1] || null
    setApprovals(prev => (prev || []).filter(a => a.id !== fileId))
  ...
  const handleApprove = async () => {
    ...
    if (selected.docType === 'certified_payroll') {
    ...
    try {
      const res = await fetch(
        `/api/documents/${encodeURIComponent(fileId)}/approve`,
        { method: 'POST' })
      const data = await res.json()
      if (data.error) throw new Error(data.error)
  ...
  const doSkipSignoff = async () => {
    ...
    try {
      const res = await fetch(
        `/api/documents/${encodeURIComponent(fileId)}/skip-signoff`,
        { method: 'POST' })
      const data = await res.json()
      if (data.error) throw new Error(data.error)
      removeApprovedAndAdvance(fileId)
  ...
  const handleSignedApproved = () => {
    ...
    removeApprovedAndAdvance(signingItem.id)
    setSigningItem(null)
  }
  ...
  const openCertPayrollSignoff = () => {
    ...
    setSigningItem({
      ...selected,
      signUrl: `/api/documents/${encodeURIComponent(selected.id)}/approve-with-signature`,
    })
  }
  ...
  const fetchFileSig = async (fileId) => {
    try {
      const m = await fetch(`/api/documents/${encodeURIComponent(fileId)}/meta`)
        .then(r => r.json())
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/documents/${encodeURIComponent(fileId)}/regenerate`,
        { method: 'POST' })
      const data = await res.json().catch(() => ({}))
      if (!res.ok || data.error) throw new Error(data.error || `HTTP ${res.status}`)
  ...
  const openInDrive = () => {
    if (!selected) return
    // Drive view not available in new system
  }
    ...
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {DOC_TYPES.map(dt => {
          const n = dt.value === 'all'
            ? pendingCount
            : (approvals || []).filter(a => a.docType === dt.value).length
        ...
        >
          ...
          {selected && (
            <>
              <div className="flex items-start justify-between gap-3 mb-3 flex-wrap">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-slate-500 mt-0.5 truncate">
                    {docTypeMeta(selected.docType).label}
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {canRegenerate(selected) && (
                    <button
                      ...
                      className="text-xs font-bold px-3 py-1.5 rounded-lg
                                 ...
                                 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {regenBusyId === selected.id ? 'Regenerating…' : 'Regenerate'}
                    </button>
                  )}
                  <button
                    type="button"
                  ...
                  >
                    {approving
                      ? 'Approving…'
                      : (isManualSignIn(selected) || selected.docType === 'certified_payroll')
                          ? 'Approve (skip sign-off)'
                          : selected.docType === 'signin'
                              ? 'Approve & Sign'
                              : 'Approve'}
                  </button>
                  {/* Sign-in overrides — kebab keeps the rare alternate
                      action available without cluttering the row. */}
                  {selected.docType === 'signin' && !approving && (
                    <RowKebab items={
                  ...
                  {selected.docType === 'certified_payroll' && !approving && (
                    <RowKebab items={[{
                      label:   'Approve with sign-off',
              ...
              <PDFViewer fileId={selected.id} width={viewerWidth} version={pdfVersion} />
            </>
          )}
        </section>
      </div>
      ...
      {signingItem && (
        <PrincipalSignModal
          ...
          signUrl={signingItem.signUrl ||
            `/api/documents/${encodeURIComponent(signingItem.id)}/approve-with-signature`}
      ...
      {reuploadOpen && selected && (
        <ReuploadModal
          fileId={selected.id}
          filename={selected.filename}
          allowScan={selected.docType === 'signin'}
      ...
      {regenConfirmOpen && selected && (
        <ConfirmModal
          ...
          message={
            `This rebuilds the ${docTypeMeta(selected.docType).label} from the current ` +
...
function PDFViewer({ fileId, width, version = 0 }) {
  ...
  const file = useMemo(() => ({
    url: `/api/documents/${encodeURIComponent(fileId)}/pdf${version ? `?v=${version}` : ''}`,
  }), [fileId, version])
