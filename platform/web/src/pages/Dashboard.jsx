    ...
    setModalAction({
      ...
      onConfirm: async () => {
        const res = await fetch('/api/tools/certified-payroll', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ weekStart: isoWeekStart }),
        })
        const data = await res.json().catch(() => ({}))
        if (!res.ok || data.error) throw new Error(data.error || `HTTP ${res.status}`)
...
function WORow({ wo, flagged, qbConnected, onDocsChange, onChangeStatus, onDeleteWO, onInvoiced }) {
  ...
  const qtyText = (!wo.quantity || isNaN(qtyNum) || qtyNum === 0)
    ? '—'
    : `${wo.quantity} ${wo.quantityUnit || ''}`.trim()

  // Progress cell: thin bar + "x / y" label. 0 / 0 → em-dash.
  const total = wo.markingsTotal ?? 0
  const done  = wo.markingsCompleted ?? 0
  ...
  return (
    <tr className={`border-b border-slate-100 text-sm transition-colors
      ...
      <td className="py-2.5 px-3 text-slate-700">{wo.contractorName || '—'}</td>
...
export default function Dashboard() {
  ...
  const onDocsChange = async (woId, friendlyDocType, partial) => {
    ...
    try {
      const res = await fetch('/api/documents/flags', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ updates: [{ woId: woId, doc_type: friendlyDocType, ...partial }] }),
      })
  ...
  function isCompletedTodayLocal(wo) {
    ...
    const raw = String(wo.workEndDate || '')
    if (!raw) return false
  ...
  const filteredWOs = useMemo(() => {
    ...
    return data.wos.filter(wo => {
      ...
      if (contFilt.size    > 0 && !contFilt.has(wo.contractorName))   return false
      if (boroughFilt.size > 0 && !boroughFilt.has(wo.regionCode))   return false
      if (docFilt.size > 0) {
        let ok = false
  ...
  const contractors = useMemo(() =>
    [...new Set((data?.wos ?? []).map(w => w.contractorName).filter(Boolean))].sort()
  , [data])

  const boroughs = useMemo(() =>
    [...new Set((data?.wos ?? []).map(w => w.regionCode).filter(Boolean))].sort()
  , [data])
  ...
  const chartData = data ? [
    ...
    { name: 'In Progress', value: data.stats.inProgress, fill: '#f97316' },
    { name: 'Completed',   value: data.stats.completed,    fill: '#22c55e' },
  ] : []
              ...
              {filteredWOs.length === 0 ? (
                ...
                filteredWOs.slice(0, visibleCount).map(wo => (
                  <WORow
                    key={wo.woNumber}
                    wo={wo}
                    flagged={attention.includes(wo.woNumber)}
                    qbConnected={!!qbStatus?.connected}
                    onDocsChange={onDocsChange}
