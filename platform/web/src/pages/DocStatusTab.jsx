...
async function fetchDocStatus(monthIso) {
  const res = await fetch('/api/documents/status?month=' + encodeURIComponent(monthIso))
...
async function flipDocFlags(updates) {
  const res = await fetch('/api/documents/status/flags', {
    method:  'POST',
            ...
            return (
              <div key={i} className={`border rounded-lg p-3 space-y-2 ${rowBg}`}>
                <div className="flex justify-between items-baseline">
                  <p className="font-semibold text-sm text-slate-800">{b.contractorName}</p>
                  {b.crewChief && (
                    <p className="text-[10px] text-slate-600">
                      <span className="font-semibold">Crew:</span> {b.crewChief}
                    </p>
                  )}
                  {contracts.length > 1 && (
          ...
          {(cell.breakdown || []).map((b, i) => {
            ...
            return (
              <div key={i} className={`border rounded-lg p-3 space-y-2 ${rowBg}`}>
                <div className="flex justify-between items-baseline">
                  <p className="font-semibold text-sm text-slate-800">{b.contractorName}</p>
                  <p className="text-xs text-slate-500 font-mono">{b.contractNum} · {b.regionCode}</p>
                </div>
                {b.woIds?.length > 0 && (
            ...
            <div className="pt-2 border-t-2 border-slate-200 space-y-3">
              ...
              {meRows.map((row, i) => {
                ...
                return (
                  <div key={i} className={`border rounded-lg p-3 space-y-2 ${rowBg}`}>
                    <div className="flex justify-between items-baseline">
                      <p className="font-semibold text-sm text-slate-800">{row.contractorName}</p>
                      <p className="text-xs text-slate-500 font-mono">{row.contractNum} · {row.regionCode}</p>
                    </div>
                    {row.contractId && (
                      <p className="text-[11px] text-slate-500">
                        <span className="font-semibold">Contract ID:</span> <span className="font-mono">{row.contractId}</span>
                      </p>
                    )}
                    {row.utilization && (
          ...
          <div key={i} className="px-3 py-2.5 flex items-center justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-slate-500 truncate">
                {it.contractNum
                  ? <>{it.contractorName} · <span className="font-mono">{it.contractNum}</span> · {it.regionCode}</>
                  : it.contractorName}
                {it.crewChief && (
                  <> · <span className="font-semibold text-slate-600">{it.crewChief}</span></>
                )}
              </p>
            </div>
...
export default function DocStatusTab({ wos, qbConnected, onDocsChange, onInvoiced }) {
  ...
  const downloadMonthEndDoc = async (item) => {
    try {
      const res = await fetch('/api/tools/month-end', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const downloadMonthEndAll = async (docIds) => {
    ...
    try {
      const res = await fetch('/api/tools/month-end/all', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        description={pendingItem ? (
          <span>
            ...
            <span className="block text-slate-500">
              {pendingItem.contractNum
                ? `${pendingItem.contractorName} · ${pendingItem.contractNum} · ${pendingItem.regionCode}`
                : pendingItem.contractorName}
            </span>
