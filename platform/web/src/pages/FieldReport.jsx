...
function WOCombobox({ wos, selectedWOId, onSelect, disabled = false, disabledTitle = '' }) {
  ...
  const labelOf  = (wo) => `${wo.woNumber} — ${wo.location} (${wo.regionCode})`

  // Filter out completed/returned WOs and sort by WO number
  const sortedWOs = useMemo(() => {
    return [...wos]
      .filter(wo => wo.status !== 'completed' && wo.status !== 'returned')
      .sort((a, b) => {
        const na = parseInt(String(a.woNumber || '').match(/\d+/)?.[0] ?? '', 10)
        const nb = parseInt(String(b.woNumber || '').match(/\d+/)?.[0] ?? '', 10)
        if (Number.isNaN(na) && Number.isNaN(nb)) return String(a.woNumber).localeCompare(String(b.woNumber))
  ...
  const filtered = !trimmed
    ? sortedWOs
    : sortedWOs.filter(wo =>
        `${wo.woNumber} ${wo.location || ''} ${wo.regionCode || ''}`
          .toLowerCase()
          .includes(trimmed)
      )
...
function MarkingItemRow({
  ...
  const ColorBox = (
    <input type="text" placeholder="Color / Material"
      value={item.colorMaterial ?? ''}
      onChange={onLocalText('colorMaterial')}
      onBlur={onCommitText('colorMaterial')}
      onKeyDown={onEnterBlur}
      readOnly={locked}
      className={locked ? LOCK : INPUT}
    />
  ...
  return (
    <div className="bg-slate-50 rounded-xl p-3 border border-slate-200 space-y-1.5">
      ...
      {[
        ...
        ['From → To',  [wo.fromStreet,wo.toStreet].filter(Boolean).join(' → ')||'—'],
        ['Due Date',   formatDueDate(wo.dueDate)],
...
export default function FieldReport() {
  ...
  const computeBufferDiff = () => {
    ...
    const FIELDS = ['category','intersection','direction','description',
                    'quantity','unit','colorMaterial','notes']
  ...
  // case.
  const isMMAJob    = selectedWO?.waterBlastRequired === 'Yes - MMA'
  const wbConfirmed = selectedWO?.waterBlastConfirmed === 'Yes'
  ...
  useEffect(() => {
    ...
    // Deep-link can be a WO number (RM-43282) or UUID — match either
    const match = wos.find(w => w.id === deepLinkedWO || w.woNumber === deepLinkedWO)
    if (match) setSelectedWOId(match.id)
  ...
  async function doSetWaterblasting(nextValue) {
    ...
    try {
      const res = await fetch(`/api/wos/${encodeURIComponent(selectedWOId)}/waterblast/confirm`, {
        method:  'POST',
  ...
  useEffect(() => {
    ...
    fetch(`/api/wos/${encodeURIComponent(selectedWOId)}/markings`)
      ...
      .then(d => {
        if (cancelled) return
        if (d.error) throw new Error(d.error)
        // Map API field names (id → item_id) so the rest of the UI works
        const items = (d.items || []).map(i => ({ ...i, item_id: i.item_id || i.id }))
  ...
  const refetchMarkings = async () => {
    if (!selectedWOId) return
    try {
      const r = await fetch(`/api/wos/${encodeURIComponent(selectedWOId)}/markings`)
  ...
  const saveField = async (itemId, patch) => {
    ...
    setRowError('')
    try {
      const res = await fetch(`/api/markings/${encodeURIComponent(itemId)}`, {
        method:  'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.item) {
        setMarkingItems(list => list.map(i => {
          ...
          const merged = {
            ...
            dateCompleted: data.item.dateCompleted,
          }
  ...
  const onRowCommit = (itemId, field, value) => {
    ...
    // Include the derived unit if it's auto-locked for this category
    // (e.g. HVX Crosswalk → LF, Stop Msg → EA). Without this, the unit
    // stays null in the DB even though the UI shows the correct one.
    const patch = { quantity: normalized }
    const derivedUnit = unitForCategory(category)
    if (derivedUnit && !item?.unit) {
      patch.unit = derivedUnit
    }
  ...
  const inferredWorkType = markingItems.some(i => String(i.workType).toLowerCase() === 'thermo')
    ? 'Thermo'
    : markingItems.some(i => String(i.workType).toLowerCase() === 'mma') ? 'MMA' : ''
  ...
  async function doSubmit() {
    ...
    if (!skipShiftCheck && crewChief.trim()) {
      try {
        ...
        const res = await fetch('/api/field-reports/check-shift', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ crewChief: crewChief.trim(), work_date: effDate }),
    ...
    try {
      ...
      const data = await res.json()
      if (data.error) throw new Error(data.error)
      setSubmitted({ woId:data.woId, status:data.status, photos:photoQueue.uploadedCount })
      ...
      // CFR document generation is now handled server-side by the
      // orchestration service during the submit call above — no separate
      // finalize request needed.
    } catch (err) {
  ...
  function handleSubmit(e) {
    ...
    const missingColor = markingItems.filter(i =>
      rowRequiresColor(i) && !String(i.colorMaterial || '').trim()
    )
  ...
  async function submitEditCompleted(mode) {
    ...
    try {
      ...
      const res = await fetch(`/api/wos/${encodeURIComponent(selectedWOId)}`, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (submitted) return (
    <div className="max-w-lg mx-auto px-4 py-16 text-center">
      ...
      <p className="text-slate-500 mb-1">
        <strong className="font-mono text-navy">{submitted.woId}</strong>
        {' '}— queued for sign-in
      </p>
      {submitted.photos > 0 && (
          ...
          {!selectedWOId ? (
            ...
            <div className="space-y-2">
              <p className="text-sm text-slate-500 italic">
                ...
                never attached.
                {selectedWO?.folderUrl
                  ? <> Or use the <a href={selectedWO.folderUrl} target="_blank" rel="noopener noreferrer" className="font-bold text-navy hover:underline">View WO 📁</a> link to drop them into Drive directly.</>
                  : null}
              </p>
              <button
                type="button"
