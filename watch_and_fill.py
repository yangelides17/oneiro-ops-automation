#!/usr/bin/env python3
"""
watch_and_fill.py  —  Oneiro Operations local PDF filler
=========================================================
...
to the same Drive folder.  No Google Drive for Desktop required.

FIRST-TIME SETUP (run once)
  1. Follow the Google Cloud setup in SETUP.md (takes ~5 min)
  2. Place credentials.json in this folder
  3. Run:  python3 watch_and_fill.py --setup
     This opens a browser login once, saves a token, then finds your
     Needs Review folder IDs automatically.
  4. Run:  python3 watch_and_fill.py
     Leave it running in a Terminal tab while you work.

USAGE
  python3 watch_and_fill.py             # run watcher
  python3 watch_and_fill.py --setup     # authenticate + discover folder IDs
  python3 watch_and_fill.py --once      # process pending files then exit

POLL INTERVAL
  Edit POLL_SECONDS below (default 20s).
"""
...
CREDS_FILE      = SCRIPT_DIR / 'credentials.json'      # downloaded from Google Cloud
TOKEN_FILE      = SCRIPT_DIR / '.token.json'            # auto-saved after first login
STATE_FILE      = SCRIPT_DIR / '.processed_files.json'  # tracks already-filled file IDs
TEMPLATE_CACHE  = SCRIPT_DIR / '_template_cache'        # local copies of Drive templates
POLL_SECONDS    = 20
...
def get_template(service, doc_type: str) -> Path:
    ...
    config_path = SCRIPT_DIR / '.drive_config.json'
